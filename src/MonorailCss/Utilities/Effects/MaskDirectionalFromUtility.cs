...
/// <summary>
/// Handles mask directional from color utilities.
/// Handles: mask-l-from-*, mask-r-from-*, mask-t-from-*, mask-b-from-*, mask-x-from-*, mask-y-from-*.
/// </summary>
internal class MaskDirectionalFromUtility : IUtility
{
    ...
    public bool TryCompile(Candidate candidate, Theme.Theme theme, out ImmutableList<AstNode>? results)
    {
        ...
        // Parse the pattern (e.g., mask-b-from, mask-x-from)
        var parts = functionalUtility.Root.Split('-');
        if (parts.Length != 3 || parts[0] != "mask" || parts[2] != "from")
        {
            return false;
        }
        ...
        if (direction == "x")
        {
            ...
            declarations.Add(new Declaration("--tw-mask-left-from-color", colorValue, candidate.Important));
            declarations.Add(new Declaration("--tw-mask-right", rightGradient, candidate.Important));
            declarations.Add(new Declaration("--tw-mask-right-from-color", colorValue, candidate.Important));
        }
        else if (direction == "y")
        {
            ...
            declarations.Add(new Declaration("--tw-mask-top-from-color", colorValue, candidate.Important));
            declarations.Add(new Declaration("--tw-mask-bottom", bottomGradient, candidate.Important));
            declarations.Add(new Declaration("--tw-mask-bottom-from-color", colorValue, candidate.Important));
        }
        else
        {
            // Single direction
            ...
            declarations.Add(new Declaration(
                ...
                candidate.Important));
            declarations.Add(new Declaration(fromColorVar, colorValue, candidate.Important));
        }
