...
{
    protected override string[] Patterns => ["backdrop-opacity"];
    protected override string[] ThemeKeys => NamespaceResolver.BackdropOpacityChain;
    protected override string FilterVariableName => "opacity";
    ...
    protected override bool TryResolveValue(CandidateValue value, Theme.Theme theme, bool isNegative, out string resolvedValue)
    {
        ...
        if (value.Kind == ValueKind.Arbitrary)
        {
            ...
            if (IsValidOpacityValue(arbitrary))
            {
                // Wrap in opacity() function if not already wrapped
                resolvedValue = arbitrary.StartsWith("opacity(") ? arbitrary : $"opacity({arbitrary})";
                return true;
            }
        ...
        if (value.Kind == ValueKind.Named)
        {
            ...
            // Convert numeric values to percentages (e.g., 50 -> 50%, 100 -> 100%)
            if (int.TryParse(key, out var numericValue))
            {
                // Keep as percentage (e.g., 50 -> 50%, 100 -> 100%)
                resolvedValue = $"opacity({numericValue}%)";
                return true;
            }
            ...
            {
                resolvedValue = $"opacity({themeValue})";
                return true;
            }
        }
    ...
    /// <summary>
    /// Validates if the arbitrary value is a valid opacity value.
    /// </summary>
    private static bool IsValidOpacityValue(string value)
    {
        ...
        if (value.StartsWith("var(") || value.Contains("calc(") || value.StartsWith("opacity("))
        {
            return true;
        }
    ...
    private static bool IsValidNumericValue(string value)
    {
        ...
        {
            var numPart = value[..^1];
            return float.TryParse(numPart, out var f) && f >= 0 && f <= 100;
        }

        // Handle decimal numbers (0 to 1)
    ...
    {
        return IsValidOpacityValue(value);
    }

    public bool TryCompile(Candidate candidate, Theme.Theme theme, CssPropertyRegistry propertyRegistry, out ImmutableList<AstNode>? results)
    {
        // Register CSS variables for backdrop filter including opacity
