...
/// <summary>
/// The scrollbar-thumb color plugin.
/// </summary>
public class ScrollbarThumb : IUtilityNamespacePlugin
{
    ...
    /// <summary>
    /// Initializes a new instance of the <see cref="ScrollbarThumb"/> class.
    /// </summary>
    /// <param name="designSystem">The design system.</param>
    public ScrollbarThumb(DesignSystem designSystem)
    {
    ...
    public IEnumerable<CssRuleSet> Process(IParsedClassNameSyntax syntax)
    {
        ...
        switch (syntax)
        {
            case ArbitraryValueSyntax arbitraryValueSyntax when arbitraryValueSyntax.Namespace.Equals("scrollbar-thumb"):
                {
                    var color = new CssColor(arbitraryValueSyntax.ArbitraryValue);
                    var declarations = new CssDeclarationList
                    {
                        (thumbVar, color.AsString()),
                        ("scrollbar-color", $"var({thumbVar}) var({trackVar}, initial)")
                    };
                    yield return new CssRuleSet(arbitraryValueSyntax.OriginalSyntax, declarations);
                    break;
                }

            case NamespaceSyntax namespaceSyntax when namespaceSyntax.NamespaceEquals("scrollbar-thumb"):
                {
    ...
    public IEnumerable<CssRuleSet> GetAllRules()
    {
        ...
        {
            var colorName = $"scrollbar-thumb-{flattenedColor.Key}";
            yield return new CssRuleSet(colorName, GetDeclarations(flattenedColor.Value, null, thumbVar, trackVar));
        }
    }

    /// <inheritdoc />
    public ImmutableArray<string> Namespaces => [..new[] { "scrollbar-thumb" }];
    ...
    private CssDeclarationList GetDeclarations(CssColor color, string? opacityValue, string thumbVar, string trackVar)
    {
        ...
        else
        {
            ...
            declarations =
            [
                (thumbVar, color.AsStringWithOpacity(opacity)),
                ("scrollbar-color", $"var({thumbVar}) var({trackVar}, initial)")
            ];
        }
